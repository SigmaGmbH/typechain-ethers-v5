...
import type { PublicConfig as RunTypeChainConfig } from 'swisstronik-typechain'
...
subtask(TASK_TYPECHAIN_GENERATE_TYPES)
  ...
  .setAction(async ({ compileSolOutput, quiet }, { config, artifacts }) => {
    ...
    const { glob } = await import('swisstronik-typechain')
    ...
    const { runTypeChain } = await import('swisstronik-typechain')
